...
// Dev-only logger helpers to avoid no-console lint warnings in production builds
const devError = (...args) => {
...
const pages = import.meta.glob('./questionnaire/Pages/**/*.vue', {
    eager: true,
})
...
const appElement =
    document.getElementById('questionnaire-app') ||
...
if (appElement) {
    createInertiaApp({
        ...
        resolve: (name) => {
            ...
            if (!page) {
                devError(`Page not found: ${normalizedName}`)
                devInfo('Available pages:', Object.keys(pages))
                throw new Error(
                    `Page ${normalizedName} not found in questionnaire package.`
                )
            }
        ...
        setup({ el, App, props, plugin }) {
            ...
            app.config.errorHandler = (err, instance, info) => {
                devError('Vue Error:', err)
                devError('Component:', instance)
                devError('Error Info:', info)
        ...
        devError('Failed to mount Inertia app:', error)
    })
}
